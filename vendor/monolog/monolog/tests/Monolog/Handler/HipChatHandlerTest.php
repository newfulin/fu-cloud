...
class HipChatHandlerTest extends TestCase
{
    ...
    {
        $this->createHandler('myToken', 'room1', 'Monolog', false, 'hipchat.foo.bar', '8.0.5');
        $this->handler->handle($this->getRecord(Logger::CRITICAL, 'test1'));
        fseek($this->res, 0);
        $content = fread($this->res, 1024);

        $this->assertRegexp('/POST \/8.0.5\/room\/room1\/notification\?auth_token=.* HTTP\/1.1\\r\\nHost: hipchat.foo.bar\\r\\nContent-Type: application\/x-www-form-urlencoded\\r\\nContent-Length: \d{2,4}\\r\\n\\r\\n/', $content);
    ...
    {
        $this->createHandler('myToken', 'room1', 'Monolog', true, 'hipchat.foo.bar', '8.0.5');
        $this->handler->handle($this->getRecord(Logger::CRITICAL, 'test1'));
        fseek($this->res, 0);
        $content = fread($this->res, 1024);
    ...
    {
        $this->createHandler('myToken', 'room name', 'Monolog', false, 'hipchat.foo.bar', '8.0.5');
        $this->handler->handle($this->getRecord(Logger::CRITICAL, 'test1'));
        fseek($this->res, 0);
        $content = fread($this->res, 1024);

        $this->assertRegexp('/POST \/8.0.5\/room\/room%20name\/notification\?auth_token=.* HTTP\/1.1\\r\\nHost: hipchat.foo.bar\\r\\nContent-Type: application\/x-www-form-urlencoded\\r\\nContent-Length: \d{2,4}\\r\\n\\r\\n/', $content);
    ...
    {
        $this->createHandler('myToken', 'room1', null, false, 'hipchat.foo.bar', '8.0.5');
        $this->handler->handle($this->getRecord(Logger::CRITICAL, 'test1'));
        fseek($this->res, 0);
        $content = fread($this->res, 1024);
    ...
    {
        // creating a handler with too long of a name but using the 8.0.5 api doesn't matter.
        $hipChatHandler = new HipChatHandler('token', 'room', 'SixteenCharsHere', false, Logger::CRITICAL, true, true, 'test', 'api.hipchat.com', '8.0.5');
    }
}
