...
class GoodsOrderService extends Service {
	...
	public $wx;
	public $balance;
	public function __construct(GoodsOrderRepo $order, CommUserRepo $user, WxUserInfoRepo $wx, AcctAccountBalanceRepository $balance, GoodsInfoRepo $info, ShoppingCarRepo $shopCard, ReceiveAddressRepo $address) {
		$this->order = $order;
		$this->user = $user;
		$this->wx = $wx;
	...
	public function getOrderDetail($request) {
		$id = $request['id'];
		return $this->order->getAddressByOrderId($id);
		$goodsInfo = $this->order->getOrderDetail($id);
		foreach ($goodsInfo as $key => $val) {
			$goodsInfo[$key]['stateNacme'] = config('common.goods_order.' . $val['state']);
		}
		$ret['goodsInfo'] = $goodsInfo;
		$ret['address'] = $goodsInfo;
		// $re['stateNacme'] = config('common.goods_order.' . $re['state']);
		return $ret;
	}
	//生成商品订单
	...
	public function batchGenerateGoodsOrder($request) {
		//获取收货地址
		// $addressInfo = $this->address->getRecInfo($request['address_id']);

		//根据购物车ID,获取商品详情
		$shop_id = array_filter(explode(',', $request['shop_id']));

		$goodsInfo = $this->shopCard->getGoodsInfoByShopId($shop_id);

		$order_id = ID();
		$data = [];
		$total_price = 0;
		foreach ($goodsInfo as $key => $val) {
			$total_price += Money()->mul($val['num'], $val['price']);
		}
		...
		foreach ($goodsInfo as $key => $val) {
			$data[] = [
				...
				// 'consignee_name' => $addressInfo['name'],
				// 'consignee_mobile' => $addressInfo['tel'],
				'user_id' => $request['user_id'],
				'goods_type' => $val['goods_info']['goods_type'],
				'goods_class' => 10,
				'from_way' => 10,
				'store_id' => $val['goods_info']['business_info']['id'],
			];
		}
	...
	//批量商品订单支付
	public function batchPayGoodsOrder($request) {
		//获取用户详情
		...
		if (!$orderInfo || $orderInfo->state == '20') {
			Err('订单已支付,请忽重复提交!');
		}

		$ret = Trans::service('ChannelTrans')
			->with('business_code', 'A0600')
			->with('trans_amt', $orderInfo->total_price)
