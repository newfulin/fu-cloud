...
class GoodsOrderRepo extends Repository
{
    ...
    public function getGoodsInfoByOrderId($request){
        return DB::table('goods_order as t0')
            ->select('t0.id','t0.goods_id','t0.number','t0.state','t0.goods_class','t1.name','t1.introduce','t1.price','t1.goods_type','t1.from_way','t1.url','t1.img')
            ->leftJoin('goods_info as t1',function($join){
                $join->on('t0.goods_id', '=', 't1.id');
            })
    ...
    public function getOrderDetail($id){
        return optional($this->model
            ->select('id','create_time','state','total_price','goods_id','number','goods_class')
            ->where('id',$id)
            ->first())
            ->toArray();
    }
