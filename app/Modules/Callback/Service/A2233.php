...
class A2233 extends Service
{
    ...
    public function update(TranTransOrderRepo $order,CommUserRepo $user,InviteCodeRepo $code,$request){
        Log::info(' 原六个车车巢转为合伙人 | '.$request['detailId']);
        ...
        //更新用户等级
        $data = [
            'user_tariff_code' => config('const_user.CITYAGENT_USER.code'),
            'level_name' => config('const_user.CITYAGENT_USER.code')
        ];
