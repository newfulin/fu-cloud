...
class A1233 extends Service
{
    ...
    public function update(TranTransOrderRepo $order,CommUserRepo $user,InviteCodeRepo $code,$request){
        Log::info(' 原六个车代理商转为总代理 | '.$request['detailId']);
        ...
        //更新用户等级
        $data = [
            'user_tariff_code' => config('const_user.AGENT_USER.code'),
            'level_name' => config('const_user.AGENT_USER.code')
        ];
